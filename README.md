...
다른 디자인패턴도 넣고싶지만 이정도만해도 충분할꺼라 생각들고 괜찮은것들만 넣을예정  
...
이거랑 비슷한게 `template callback` 패턴인데 스프링에서 상당히 많이 쓰임.  
차이점은 `template method` 패턴이 상속을 사용하고 `template callback` 이 위임을 사용한다.
...
패턴이 이것도 형태가 명확한편인데 더블 디스페쳐(double dispacher)가 일어난다고 생각하면 쉽다.  
이게 무슨말이냐면 A 가 컴파일중에 실제 구현체가 무엇인지 찾고, B 도 마찬가지로 컴파일중에 실제 구현체가 무엇인지 찾는다.  
이것 외에도 B 가 A 의 구현체별로 메소드 인자를 만들어야한다.  
위와 같이 2개의 특징을 통해 if 문을 쉽게 제거할수있다.  
다만 해당 패턴을 처음 봤을때에는 이해하기가 진짜 쉽지 않았다.....  
~~예전에 다른 프로젝트 보다가 난 왜이렇게 빠가사리일까 싶을정도로...~~ 무튼 잘 사용하면 좋을꺼같지만  
실제로 어떤 상황에서 적용할수있까 생각해보면 그닥 바로 떠오르는게 없다.
